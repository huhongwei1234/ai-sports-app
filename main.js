...
protocol.registerSchemesAsPrivileged([
    ...
    { scheme: 'res', privileges: { secure: true, standard: true, supportFetchAPI: true, corsEnabled: true } }
]);
...
function createWindow() {
    ...
    // 新路径：node_modules/_internal/www/index.html
...
app.whenReady().then(async () => {
    ...
    // 注册 res:// 协议，指向 Resources 目录（logo.jpg / config.json）
    protocol.handle('res', (request) => {
        const url = new URL(request.url);
        let pathname = decodeURIComponent(url.pathname);
        if (pathname.startsWith('/')) pathname = pathname.slice(1);
        const filePath = path.join(process.resourcesPath, pathname);
        // 安全检查
        if (!filePath.startsWith(process.resourcesPath)) {
            return new Response('Forbidden', { status: 403 });
        }
        let data;
        try {
